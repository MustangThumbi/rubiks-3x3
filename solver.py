...
def main(root):
    ...
    #OPTIONS
    Button(options,width=10,text="RESET",command=lambda:reset(opt,mode)).grid(row=0,column=0)
    Button(options,width=17,text="SOLUTION",command=lambda:solve(opt)).grid(row=1,column=0)
